...
export default function Match() {
  ...
  const [matchScores, setMatchScores] = useState<Record<string, { team1: number; team2: number }>>({});
  ...
  const getBackendMatchType = () => {
    switch (matchFormat) {
      case "roundrobin": return "round-robin";
      case "balanced": return "skill-based";
      case "random": return "skill-based"; // Map random to skill-based as fallback
      default: return "skill-based";
    }
  };
  ...
  const generateTournament = async () => {
    ...
    try {
      ...
      alert("Có lỗi xảy ra khi tạo giải đấu. Vui lòng thử lại.");
    } finally {
  ...
  const submitMatchResult = async (match: any) => {
    ...
    if (!scores || (scores.team1 === undefined || scores.team2 === undefined)) {
      alert("Vui lòng nhập điểm số cho cả hai đội");
      return;
    }
    ...
    try {
      ...
      setMatches(prev => prev.map(m => 
        m._id === match._id 
          ? { ...m, status: 'completed' as const, team1: { ...m.team1, score: scores.team1 }, team2: { ...m.team2, score: scores.team2 } }
          : m
      ));
      ...
      alert("Có lỗi xảy ra khi cập nhật kết quả. Vui lòng thử lại.");
    }
  };

  const getTeamSkillAverage = (playerIds: string[]) => {
    const teamPlayers = selectedPlayers.filter(p => playerIds.includes(p._id));
    if (teamPlayers.length === 0) return 0;
    return teamPlayers.reduce((sum, player) => sum + player.skillPoints, 0) / teamPlayers.length;
  };
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
        <div className="container max-w-7xl px-6 py-8 mx-auto">
          <Card className="text-center py-16 bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700">
            <CardHeader>
              <CardTitle className="text-2xl mb-4 text-gray-900 dark:text-slate-100">Chưa chọn người chơi</CardTitle>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <Card className="bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700">
                <CardHeader>
                  <CardTitle className="text-lg text-gray-900 dark:text-slate-100">Thông tin giải đấu</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  <div className="text-sm text-gray-600 dark:text-slate-400 space-y-1">
                    ...
                    <div>Số đội dự kiến: {Math.floor(selectedPlayers.length / 2)}</div>
                    {matchFormat === "roundrobin" && (
                      <div>Số trận đấu: {Math.floor(selectedPlayers.length / 2) * (Math.floor(selectedPlayers.length / 2) - 1) / 2}</div>
                    )}
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800">
              <CardHeader>
                ...
                <CardDescription className="text-blue-700 dark:text-blue-300">
                  {matches.length} trận đấu • {selectedPlayers.length} người chơi
                </CardDescription>
              </CardHeader>
            </Card>
            ...
            <div className="space-y-4">
              {matches.map((match, index) => {
                ...
                const matchScore = matchScores[match._id] || { team1: 0, team2: 0 };

                return (
                  <Card key={match._id} className="bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700">
                    <CardHeader className="pb-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className={`px-3 py-1 rounded-full text-xs font-medium ${
                          ...
                          match.status === "playing" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400" :
                          "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                        }`}>
                          {match.status === "pending" ? "Chờ đấu" : match.status === "playing" ? "Đang đấu" : "Hoàn thành"}
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm text-gray-500 dark:text-slate-400">
                              TB: {getTeamSkillAverage(match.team1.players).toFixed(1)}
                            </span>
                          </div>
                          <div className="space-y-2">
                            {team1Players.map((player) => (
                              <div key={player._id} className="flex items-center justify-between p-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded">
                                <div>
                                  <div className="font-medium text-sm text-gray-900 dark:text-slate-100">{player.name}</div>
                                  <div className="text-xs text-gray-500 dark:text-slate-400">{player.gender === "Male" ? "Nam" : "Nữ"}</div>
                                </div>
                                <span className="text-sm font-mono text-gray-900 dark:text-slate-100">{player.skillPoints}</span>
                              </div>
                            ))}
                          </div>
                          ...
                          {match.status !== "completed" && (
                            <div className="mt-2">
                              <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Điểm số:</label>
                              <input
                                type="number"
                                min="0"
                                value={matchScore.team1}
                                onChange={(e) => handleScoreChange(match._id, 'team1', parseInt(e.target.value) || 0)}
                          ...
                          {/* Display final score */}
                          {match.status === "completed" && match.team1.score !== undefined && (
                        ...
                        <div className="text-center">
                          <div className="text-xl font-bold text-gray-400 dark:text-slate-500">VS</div>
                        </div>
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm text-gray-500 dark:text-slate-400">
                              TB: {getTeamSkillAverage(match.team2.players).toFixed(1)}
                            </span>
                          </div>
                          <div className="space-y-2">
                            {team2Players.map((player) => (
                              <div key={player._id} className="flex items-center justify-between p-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded">
                                <div>
                                  <div className="font-medium text-sm text-gray-900 dark:text-slate-100">{player.name}</div>
                                  <div className="text-xs text-gray-500 dark:text-slate-400">{player.gender === "Male" ? "Nam" : "Nữ"}</div>
                                </div>
                          ...
                          {match.status !== "completed" && (
                            <div className="mt-2">
                              ...
                              <input
                                ...
                                onChange={(e) => handleScoreChange(match._id, 'team2', parseInt(e.target.value) || 0)}
                                className="w-full mt-1 px-2 py-1 border border-gray-300 dark:border-slate-600 rounded text-center bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100"
                              />
                            </div>
                          )}
                          ...
                          {/* Display final score */}
                          {match.status === "completed" && match.team2.score !== undefined && (
