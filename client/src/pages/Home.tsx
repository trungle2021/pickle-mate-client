  ...
  const handleAddPlayer = async () => {
    ...
    try {
      ...
      addToast({
        type: "success",
        title: "Thành công!",
        message: `Đã thêm người chơi "${newPlayer.name}" vào danh sách`,
        duration: 3000,
      });
    } catch (error: any) {
  ...
  const handleDeletePlayer = async (playerId: string, playerName: string) => {
    ...
    try {
      ...
      addToast({
        type: "success",
        title: "Đã xóa!",
        message: `Người chơi "${playerName}" đã được xóa khỏi danh sách`,
        duration: 3000,
      });
    } catch (error: any) {
      handleApiError(error, addToast);
  ...
  const handleResetAllPoints = async () => {
    ...
    try {
      ...
      addToast({
        ...
        message: "Điểm skill của tất cả người chơi đã được reset về 1.0",
        duration: 4000,
      });
    } catch (error: any) {
      handleApiError(error, addToast);
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewPlayer((prev) => ({
                        ...prev,
                        skillPoints: parseFloat(e.target.value) || 1.0,
                      }))
                    }
                    className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100"
                  />
